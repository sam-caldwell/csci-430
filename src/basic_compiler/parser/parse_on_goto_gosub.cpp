...
std::unique_ptr<Stmt> Parser::parseOnGotoGosub() {
    ...
    if (!check(TokenType::Integer)) throw ParseError("Expected line number after GOTO/GOSUB");
    do {
        if (!check(TokenType::Integer)) throw ParseError("Expected line number in ON dispatch list");
        int ln = std::stoi(peek().lexeme);
        advance();
        targets.push_back(ln);
    } while (match(TokenType::Comma));

    if (gosub) return make_node<OnGosubStmt>({0,0}, std::move(idx), std::move(targets));
