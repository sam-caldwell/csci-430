...
std::vector<Token> Lexer::tokenize() {
    ...
    while (!atEnd()) {
        ...
        if (tryEmitNewline(tokens)) {
            continue;
        }

        if (tryEmitPrimary(tokens)) {
            continue;
        }
        ...
        const int tcol = col_;
        const char chr = peek();
        if (chr == Symbols::AMPERSAND.first()) {
            emitAmpLiteral(tokens, tline, tcol);
            continue;
        }
        if (tryEmitOperatorOrPunct(tokens, tline, tcol, chr)) {
            continue;
        }

        std::ostringstream oss;
        oss << "Unexpected character '" << chr << "' at " << line_ << ':' << col_;
