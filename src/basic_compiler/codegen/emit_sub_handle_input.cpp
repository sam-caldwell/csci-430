...
void CodeGenerator::emitSubHandleInput(std::ostringstream& out, const InputStmt* ins, std::string_view entryLabel) {
    ...
    if (ins->promptLiteral || ins->promptVar) {
        ...
        if (ins->promptLiteral) {
            ...
            pstr = nextTemp();
            {
                std::string ir = std::format("  {} = load ptr, ptr {}", pstr, varAllocaName_[*ins->promptVar]);
                out << ir << Symbols::LF;
            }
        ...
        if (!pstr.empty()) {
            ...
            {
                std::string ir = std::format("  {} = getelementptr inbounds i8, ptr @.fmt_str_sp, i64 0", fmtS);
                out << ir << Symbols::LF;
            }
            {
    ...
    for (const auto& vname : ins->variables) {
        ...
        out << ir2 << Symbols::LF;
        log() << "line " << currentLine_ << " InputStmt -> " << ir2 << Symbols::LF;
