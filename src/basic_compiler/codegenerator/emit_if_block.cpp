...
namespace gwbasic {
    ...
    void CodeGenerator::emitIfBlock(std::ostringstream &out, const IfBlockStmt *ib, const std::string &currLineLabel,
        ...
        if (!ib->elseBody.empty()) {
            std::string br = std::format("  br i1 {}, label %{}, label %{}", cond, thenLbl, elseLbl);
            out << br << Symbols::LF;
            log() << "line " << currentLine_ << " IfBlock -> " << br << Symbols::LF;
        } else {
            std::string br = std::format("  br i1 {}, label %{}, label %{}", cond, thenLbl, endLbl);
            out << br << Symbols::LF;
            log() << "line " << currentLine_ << " IfBlock -> " << br << Symbols::LF;
        }
        ...
        for (const auto &s: ib->thenBody) {
            if (auto asg = dyn_cast<AssignStmt>(s.get())) {
                ...
                std::string n = nextTemp();
                {
                    std::string ir = std::format("  {} = call i64 @strlen(ptr {})", n, buf);
                    out << ir << Symbols::LF;
                }
                std::string size = nextTemp();
                {
                    std::string ir = std::format("  {} = add i64 {}, 1", size, n);
                    out << ir << Symbols::LF;
                }
                std::string mem = nextTemp();
                {
                    std::string ir = std::format("  {} = call ptr @malloc(i64 {})", mem, size);
                    out << ir << Symbols::LF;
                }
                {
                    std::string ir = std::format("  call ptr @strncpy(ptr {}, ptr {}, i64 {})", mem, buf, n);
                    out << ir << Symbols::LF;
                }
                std::string pn = nextTemp();
                {
                    std::string ir = std::format("  {} = getelementptr inbounds i8, ptr {}, i64 {}", pn, mem, n);
                    out << ir << Symbols::LF;
                }
                {
                    std::string ir = std::format("  store i8 0, ptr {}", pn);
                    out << ir << Symbols::LF;
                }
                ensureVarAllocated(out, li->name);
                {
                    std::string ir = std::format("  store ptr {}, ptr {}", mem, varAllocaName_[li->name]);
                    out << ir << Symbols::LF;
                }
            } else {
                ...
                if (auto asg = dyn_cast<AssignStmt>(s.get())) {
                    ...
                    for (const auto &vname: ins->variables) {
                        ...
                        std::string fmt = nextTemp();
                        {
                            std::string ir1 = std::format("  {} = getelementptr inbounds i8, ptr @.fmt_in, i64 0", fmt);
                            out << ir1 << Symbols::LF;
                            log() << "line " << currentLine_ << " IfBlock else Input -> " << ir1 << Symbols::LF;
                        }
                        std::string tmp = nextTemp();
                        {
                            std::string ir = std::format("  {} = alloca double", tmp);
