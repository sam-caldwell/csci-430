...
std::string CodeGenerator::emitExpr(std::ostringstream& out, const Expr* e, [[maybe_unused]] const std::string& currBlockSuffix) {
    ...
    if (auto call = dyn_cast<const CallExpr>(e)) {
        ...
        if (fn == "RND") { std::string ir = "  "; ir += res; ir += " = call double @gwb_rnd(double "; ir += argv[0]; ir += ")"; out << ir << "\n"; { std::ostringstream m; m << "line " << currentLine_ << " CallExpr rnd(full) -> " << ir; log(m.str()); } return res; }
        if (fn == "CINT") {
