...
bool CodeGenerator::emitForBodyStatement(std::ostringstream& out,
    ...
    if (const auto* assignStmt = dyn_cast<AssignStmt>(stmt)) {
        emitForHandleAssign(out, assignStmt, currLineLabel);
        return false;
    }
    if (const auto* printStmt = dyn_cast<PrintStmt>(stmt)) {
        emitForHandlePrint(out, printStmt, currLineLabel, localCounter);
        return false;
    }
    if (const auto* midAssign = dyn_cast<MidAssignStmt>(stmt)) {
        emitForHandleMidAssign(out, midAssign, currLineLabel, localCounter);
        return false;
    }
    if (const auto* onGoto = dyn_cast<OnGotoStmt>(stmt)) {
        emitForHandleOnGoto(out, onGoto, currLineLabel, localCounter);
        return false;
    }
    if (const auto* onGosub = dyn_cast<OnGosubStmt>(stmt)) {
        emitForHandleOnGosub(out, onGosub, currLineLabel, localCounter);
        return false;
    }
    if (const auto* gotoStmt = dyn_cast<GotoStmt>(stmt)) {
        return emitForHandleGoto(out, gotoStmt);
    }
    if (const auto* gosubStmt = dyn_cast<GosubStmt>(stmt)) {
        emitForHandleGosub(out, gosubStmt, currLineLabel, localCounter);
        return false;
    }
    if (const auto* arrAssign = dyn_cast<ArrayAssignStmt>(stmt)) {
        emitForHandleArrayAssign(out, arrAssign, currLineLabel, localCounter);
        return false;
    }
    if (isa<StopStmt>(stmt)) {
        emitForHandleStop(out);
        return true;
    }
    if (isa<SystemStmt>(stmt)) {
        emitForHandleSystem(out);
        return true;
    }
