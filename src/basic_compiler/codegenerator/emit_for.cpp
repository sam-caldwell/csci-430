 ...
 *  - void
 */
void CodeGenerator::emitFor(std::ostringstream& out, const ForStmt* for_stmt, const std::string& currLineLabel, int& localCounter) {
    ...
    const std::string stepReg = for_stmt->step ? emitExpr(out, for_stmt->step.get(), currLineLabel) : std::string("1.0");
    ...
    const bool forTerminated = emitForBodyStatements(out, for_stmt, currLineLabel, localCounter);

    // Increment and back-edge if loop not terminated in body
    if (!forTerminated) {
        out << std::format("  br label %{}", incLbl) << Symbols::LF;
        out << incLbl << ":" << Symbols::LF;
        const std::string stepReg2 = for_stmt->step ? emitExpr(out, for_stmt->step.get(), currLineLabel) : std::string("1.0");
