...
void CodeGenerator::emitFor(std::ostringstream& out, const ForStmt* fs, const std::string& currLineLabel, int& localCounter) {
        ...
        if (auto asg = dyn_cast<AssignStmt>(s.get())) {
            ...
            for (size_t pi = 0; pi < items.size(); ++pi) {
                ...
                if (isStr(v, isStr)) {
                    ...
                    { std::string ir3 = std::format("  call i32 (ptr, ...) @printf(ptr {}, ptr {})", fmt, sptr); out << ir3 << Symbols::LF; log() << "line " << currentLine_ << " ForStmt body Print -> " << ir3 << Symbols::LF; }
                } else {
                    auto val = emitExpr(out, v, currLineLabel);
                    std::string fmt = nextTemp();
                    { std::string ir1 = std::format("  {} = getelementptr inbounds i8, ptr {}, i64 0", fmt, (last ? "@.fmt_num" : "@.fmt_num_sp")); out << ir1 << Symbols::LF; log() << "line " << currentLine_ << " ForStmt body Print -> " << ir1 << Symbols::LF; }
                    { std::string ir2 = std::format("  call i32 (ptr, ...) @printf(ptr {}, double {})", fmt, val); out << ir2 << Symbols::LF; log() << "line " << currentLine_ << " ForStmt body Print -> " << ir2 << Symbols::LF; }
                }
            }
    ...
    out << "  br label %" << incLbl << Symbols::LF;
