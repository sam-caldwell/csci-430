...
# Purpose: Human-friendly help target and defaults
...
help:
	...
	@printf "  %-12s %s\n" "configure" "Generate build system files (no build)"
	@printf "  %-12s %s\n" "demo"     "Compile demos/* into build/demos/<name>/ (bin, .ll, .bc, .asm, logs)"
	...
	@printf "  %-12s %s\n" "unit"     "Run unit tests"
	@printf "  %-12s %s\n" "version"  "Show versions for CMake, Clang/LLVM, Ninja, OS, CPU"
	@printf "  %-12s %s\n" "zip"      "Archive repository into <repo>.zip, excluding build/ and VCS/IDE files"
	@echo
