 ...
 * Purpose: Ensure ON ERROR GOTO, ERROR, and the three RESUME forms parse correctly.
 * Components Under Test: Parser for error handling statements.
 * Expected Behavior: AST contains expected node kinds and resume variants.
 */
TEST(Parser, OnErrorGotoAndResumeParse) {
    std::string src =
